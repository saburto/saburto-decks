...
test.beforeEach(async ({ page }) => {
  ...
  await expect(page.locator('#deck section.slide')).toHaveCount(22)
})

test.describe('an embedded deck', () => {
  test('shows its twenty-two slides one at a time', async ({ page }) => {
    expect((await state(page)).count).toBe(22)
    ...
    for (let index = 0; index < 22; index++) {
      ...
      await expect(page.locator('#deck .counter')).toHaveText(`${index + 1} / 22`)
    }
    ...
    await expect(page.locator('#deck .counter')).toHaveText('6 / 22')
    await nextSlide(page)
    await expect(page.locator('#deck .counter')).toHaveText('7 / 22')
  })

  test('highlights code, and follows the theme (R11)', async ({ page }) => {
    await goTo(page, 4)
    await expect(page.locator('#deck .counter')).toHaveText('5 / 22')
  ...
  test('steps through a code block before leaving the slide (R11, R12)', async ({ page }) => {
    ...
    await expect(page.locator('#deck .live')).toHaveText('Slide 5 of 22, step 4 of 4')
    ...
    /* A click on the slide leaves it where it is; the keyboard and the bar
       move it (R12). */
    await goTo(page, 4)
  ...
  test('walks the full Slidev sequence, {all|4|6|6-7|9|all} (R11, R12)', async ({ page }) => {
    ...
    for (const [step, lines] of expected) {
      await page.keyboard.press('ArrowRight')
      now = await state(page)
      expect(now.index, `still on the slide at step ${step}`).toBe(5)
  ...
  test('leaves the keyboard to the page until it is focused (N1)', async ({ page }) => {
    ...
    await page.keyboard.press('ArrowRight')
    await expect(page.locator('#deck .counter')).toHaveText('2 / 22')
    await page.keyboard.press('ArrowLeft')
    await expect(page.locator('#deck .counter')).toHaveText('1 / 22')
  })
  ...
  test('the host can drive it and is told what happened (R10)', async ({ page }) => {
    ...
    await page.keyboard.press('ArrowRight')
    await expect(hostStatus(page)).toHaveText('present · slide 2/22')
    ...
    await expect(deck(page)).toHaveAttribute('data-mode', 'embedded')
    await expect(hostStatus(page)).toHaveText('embedded · slide 2/22')
  })

  test('a handle the host took early keeps working (R10)', async ({ page }) => {
    /* A host may keep the handle it was first given rather than re-read it
       after every render — the demo page and the inspect tool both do. It has
